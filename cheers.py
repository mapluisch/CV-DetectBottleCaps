...
START_FRAME_FACTOR = 0.25
END_FRAME_FACTOR = 0.75
STILLFRAME_DIFF_THRESH = 10000000
...
def get_still_frame_from_video(video_filepath):
    ...
    for i in range(start_frame+1, end_frame, fps*1):
        # skip to frame i
        ...
        # old approach: find two frames below a set threshold
        # if (ret and check_for_still_frame(last_frame, frame)):
        #     # still frame detected
        #     if (DEBUG):
        #         print("-- still frame detected after %s seconds --" % (time.time() - start_time))
        #     # generate filename and store the detected still frame (_dsf ending) into the input folder for the next steps
        #     filename = STILLFRAME_FOLDER + f'{video_filepath}_dsf.jpg'  
        #     cv2.imwrite(filename, frame)
        #     return filename
        # else:
        #     print("Error while trying to grab a still frame. Please try again.")

        # overwrite last_frame to frame for the next iteration
        last_frame = frame
    filename = STILLFRAME_FOLDER + f'{video_filepath}_dsf.jpg'  
...
def check_for_still_frame(f1, f2):
    diff = cv2.absdiff(f1, f2)
   # mask = cv2.cvtColor(diff, cv2.COLOR_BGR2GRAY)
    if(diff.sum() <= STILLFRAME_DIFF_THRESH):
        return True
